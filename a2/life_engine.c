...
{
  int indexr[8] = {0, -1, 0, 1, 1, -1, -1, 1};
  int indexc[8] = {-1, 0, 1, 0, 1, -1, 1, -1};
  int ans = 0;
  if (is_in_range(board, row, col))
  {
    for (int i = 0; i < 8; i++)
    {
      int numr = row + indexr[i];
      int numc = col + indexc[i];
      if (is_alive(board, numr, numc))
      {
        ans++;
      }
    }
...
{
  int presentcol = current.num_cols;
  int presentrow = current.num_rows;
  for (int i = 0; i < presentrow; i++)
  {
    for (int j = 0; j < presentcol; j++)
    {
      int n = count_live_nbrs(current, i, j);
